...
class UI_main(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.blocktable.setColumnCount(8)  # 8개 컬럼
    ...
    def process_file(self, db_filepath): # db에서 가져와서 보여주는
        ...
        for row in cursor.fetchall():
            ...
            rowPosition = self.blocktable.rowCount() # 현재 행의 개수
            self.blocktable.insertRow(rowPosition) # 새로운 행 추가

            for col, value in enumerate([index, name, channel, start_time, end_time, start_offset, end_offset, size]):
                item = QTableWidgetItem(str(value)) # str로 변환해서 넣음 / item = QTableWidgetItem(str(value) if col != 2 else value) 이걸로 하면 int 구분
                item.setFlags(item.flags() ^ Qt.ItemIsEditable)  # Qt.ItemIsEditable 플래그 비활성화
                self.blocktable.setItem(rowPosition, col, item) # 행, 열, QTableWidgetItem 삽입
    ...
    def update_right_side_for_precise_scan(self):
        ...
        # 오른쪽 영역 - mainLayout
        self.blocktable.setColumnCount(14)  # 14개 컬럼
        self.blocktable.setHorizontalHeaderLabels(
